--- ### miafate Buffer Utilities
--
-- Buffer management related utility functions
--
-- This module can be loaded with `local buffer_utils = require "miafate.utils.buffer"`
--
-- @module miafate.utils.buffer
-- @copyright 2022
...
function M.move(n)
	...
	require("miafate.utils").event "BufsUpdated"
...
function M.close(bufnr, force)
	...
	if require("miafate.utils").is_available "bufdelete.nvim" then
...
---@param compare_func string|function a string of a comparator defined in require("miafate.utils.buffer").comparator or a custom comparator function
...
function M.sort(compare_func, skip_autocmd)
	...
	if type(compare_func) == "function" then
		...
		vim.t.bufs = bufs
		if not skip_autocmd then require("miafate.utils").event "BufsUpdated" end
...
local function unique_path(bufnr)
	return require("miafate.utils.status").provider.unique_path() { bufnr = bufnr }
